...
float imuRollStartCur = 0, imuPitchStartCur = 0, imuYawStartCur = 0;//点云第一个点的RPY
float imuRollCur = 0, imuPitchCur = 0, imuYawCur = 0;//点云最后一个点的RPY
float imuShiftFromStartXCur = 0, imuShiftFromStartYCur = 0, imuShiftFromStartZCur = 0;//点云最后一个点相对于第一个点由于加减速产生的畸变位移
float imuVeloFromStartXCur = 0, imuVeloFromStartYCur = 0, imuVeloFromStartZCur = 0;//点云最后一个点相对于第一个点由于加减速产生的畸变速度
...
void laserCloudExtreCurHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudExtreCur2)//特征点回调函数
{
  if (!systemInited) {
    initTime = laserCloudExtreCur2->header.stamp.toSec();
    systemInited = true;
  }
  timeLaserCloudExtreCur = laserCloudExtreCur2->header.stamp.toSec();//当前时间
  timeLasted = timeLaserCloudExtreCur - initTime;//当前时间相对于起始时间的差值
  ...
  for (int i = 0; i < laserCloudExtreCur3Size; i++) {
    if (fabs(laserCloudExtreCur3->points[i].v - 10) < 0.005) {//提取当前帧imu信息
      ...
      laserCloudExtreCur->push_back(laserCloudExtreCur3->points[i]);//特征点存入laserCloudExtreCur
    }
  }
  ...
  if (timeLasted > scanWaitTime) {////todo 修改使能时间
    newLaserCloudExtreCur = true;
  }
}

void laserCloudLastHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudLast2)//所有点回调函数
{
  if (laserCloudLast2->header.stamp.toSec() > timeLaserCloudLast + 0.005) {
    ...
    for (int i = 0; i < laserCloudLastSize; i++) {
      ...
      if (fabs(laserCloudLast->points[i].v - 2) < 0.005 || fabs(laserCloudLast->points[i].v - 1) < 0.005) {
        laserCloudCornerLast->push_back(laserCloudLast->points[i]);//角点
      } 
      if (fabs(laserCloudLast->points[i].v) < 0.005 || fabs(laserCloudLast->points[i].v + 1) < 0.005) {
        laserCloudSurfLast->push_back(laserCloudLast->points[i]);//平面点
      }
      if (fabs(laserCloudLast->points[i].v - 10) < 0.005) {//imu
...
int main(int argc, char** argv)
{
  ...
  ros::NodeHandle nh;
    ...
    if(!nh.getParam("scanWaitTime",scanWaitTime))
        scanWaitTime = 1;
    std::cout<<"scanWaitTime = "<<scanWaitTime<<std::endl;
  ...
  while (status) {
    ...
    if (newLaserCloudExtreCur && newLaserCloudLast) {
      ...
      startTime = startTimeCur;//前一帧时间
      endTime = timeLasted;//当前时间
      ...
      for (int i = 0; i < 6; i++) {
        transform[i] += s * transformRec[i];  //获取当前关键帧相对于上一帧根据时间插值出来的T
      }
      timeLastedRec = timeLasted;

      newLaserPoints = true;

      if (laserCloudSurfLast->points.size() >= 100) {//判断点是否充足
        sufficientPoints = true;
      }
    }
    ...
    if (newLaserPoints) {
      ...
      bool isSendTf = true;
      if(std::isnan(laserOdometry.pose.pose.orientation.x)){
        std::cout<<"laserOdometry.pose.pose.orientation.x = "<<laserOdometry.pose.pose.orientation.x<<std::endl;
        isSendTf = false;
      ...
      if(isSendTf){
        ...
        //std::cout<<"laserOdometry.pose.pose.orientation.z = "<<laserOdometry.pose.pose.orientation.z<<std::endl<<std::endl<<std::endl;
